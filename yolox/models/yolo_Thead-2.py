...
from yolox.utils import bboxes_iou,meshgrid#bboxes_giou,bboxes_diou,bboxes_ciou,bboxes_eiou
...
class YOLOX_THead(nn.Module):
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (reg_conv, stride_this_level, x) in enumerate(
            ...
            inter_feats = []
            if k==0:
                for i, inter_conv in enumerate(self.stems1):  
                    x = inter_conv(x)
                    inter_feats.append(x)
            elif k==1:
                for i, inter_conv in enumerate(self.stems2):
                    x = inter_conv(x)
                    inter_feats.append(x)
            elif k==2:
                for i, inter_conv in enumerate(self.stems3):
                    x = inter_conv(x)
                    inter_feats.append(x)
